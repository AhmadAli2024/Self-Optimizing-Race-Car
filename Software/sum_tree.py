...
class SumTree(object):
    ...
    def add(self, priority, data):
        ...
        if self.counter != 100000:
            self.counter+=1
        # wandb.log({"Memory length":self.counter})
...
class Memory(object):  # stored as ( state, action, reward, next_state ) in SumTree
    PER_e = 0.05  # Hyperparameter that we use to avoid some experiences to have 0 probability of being taken
    ...
    def batch_update(self, tree_idx, abs_errors):
        ...
        # ps_list = []
        # # while len(ps_list) != len(tree_idx):
            # ps_list.append(ps)
